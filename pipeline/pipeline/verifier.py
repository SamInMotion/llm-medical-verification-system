...
# Singleton instances so we only load data once
_icd = None
_snomed = None
...
def verify_text(text, api_key=None):
    """
    Run the full verification pipeline on a clinical text.

    Steps:
    1. Send text to Claude for medical entity extraction
    2. For each extracted entity, verify against ICD-10 and SNOMED-CT
    3. Score each entity using the 4-signal confidence system
    4. Return structured results

    Arguments:
        text: clinical text to analyse
        api_key: Anthropic API key (falls back to env var)

    Returns:
        dict with verified_entities list, summary stats, and metadata.
    """
    ...
    # Step 2 and 3: Verify and score each entity
    verified = []
